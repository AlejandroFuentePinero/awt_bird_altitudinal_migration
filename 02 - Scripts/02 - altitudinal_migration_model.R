...
cat(file = "altitudinal_migration_awt.txt","
 ...
 #############
  ...
  beta.trend2 ~ dnorm(0,0.1) 
    ...
    #=== Mountain-segregated interaction effect (with shrinkage to borrow stregth across mountains)
    
      for(m in 1:nmountain){
        
            beta.elev_season[m] ~ dnorm(mu.beta_elev_season, tau.beta_elev_season)      
      }
      
      mu.beta_elev_season ~ dnorm(0, 0.01)
      tau.beta_elev_season <- 1 / (sigma.beta_elev_season^2)
      sigma.beta_elev_season ~ dunif(0, 10)
  ...
  for (i in 1:nsites){
      ...
      log(lambda[i,t]) <- beta0_mountain[mountain[i]] +                         # mountain-segregated interecept
                          ...
                          beta.elev_season[mountain[i]] * elev[i] * season[t] + # Interaction effect (altitudinal migration spotter)
  ...
  slope_winter[m] <- beta.elev - 0.5 * beta.elev_season[m]
  slope_summer[m] <- beta.elev + 0.5 * beta.elev_season[m]
...
inits <- function() list(N = Nst,
                         ...
                         beta.elev = runif(1),
                         beta.season = runif(1),
                         beta.elev_season = runif(bdata$nmountain),
                         beta.trend = runif(1),
                         beta.trend2 = runif(1),
                         ###
                         alpha.wind = runif(1),
...
res <- tibble(species = rep(spp_sel, 22),
              parameter = c("Intercept","Intercept","Intercept",
                            "Elevation", "Season",
                            "Elev * Season", "Elev * Season", "Elev * Season",
                            "Marginal winter slope", "Marginal winter slope", "Marginal winter slope",
                            "Marginal summer slope","Marginal summer slope","Marginal summer slope",
              ...
              average = c(round(out3$mean$beta0_exp[1],3), 
                          ...
                          round(out3$mean$beta0_exp[3],3),
                          round(out3$mean$beta.elev,3), 
                          round(out3$mean$beta.season,3),
                          round(out3$mean$beta.elev_season[1],3),
                          round(out3$mean$beta.elev_season[2],3),
                          round(out3$mean$beta.elev_season[3],3),
                          round(out3$mean$slope_winter[1],3),
                          round(out3$mean$slope_winter[2],3),
                          round(out3$mean$slope_winter[3],3),
                          round(out3$mean$slope_summer[1],3),
                          round(out3$mean$slope_summer[2],3),
                          round(out3$mean$slope_summer[3],3),
              ...
              lower_cri = c(bayestestR::ci(out3$sims.list$beta0_exp[,1], ci = 0.8)[[2]], 
                            ...
                            bayestestR::ci(out3$sims.list$beta0_exp[,3], ci = 0.8)[[2]],
                            bayestestR::ci(out3$sims.list$beta.elev, ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$beta.season, ci = 0.8)[[2]],
                            bayestestR::ci(out3$sims.list$beta.elev_season[,1], ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$beta.elev_season[,2], ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$beta.elev_season[,3], ci = 0.8)[[2]],
                            bayestestR::ci(out3$sims.list$slope_winter[,1], ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$slope_winter[,2], ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$slope_winter[,3], ci = 0.8)[[2]],
                            bayestestR::ci(out3$sims.list$slope_summer[,1], ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$slope_summer[,2], ci = 0.8)[[2]], 
                            bayestestR::ci(out3$sims.list$slope_summer[,3], ci = 0.8)[[2]],
              ...
              upper_cri = c(bayestestR::ci(out3$sims.list$beta0_exp[,1], ci = 0.8)[[3]], 
                            ...
                            bayestestR::ci(out3$sims.list$beta0_exp[,3], ci = 0.8)[[3]],
                            bayestestR::ci(out3$sims.list$beta.elev, ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$beta.season, ci = 0.8)[[3]],
                            bayestestR::ci(out3$sims.list$beta.elev_season[,1], ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$beta.elev_season[,2], ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$beta.elev_season[,3], ci = 0.8)[[3]],
                            bayestestR::ci(out3$sims.list$slope_winter[,1], ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$slope_winter[,2], ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$slope_winter[,3], ci = 0.8)[[3]],
                            bayestestR::ci(out3$sims.list$slope_summer[,1], ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$slope_summer[,2], ci = 0.8)[[3]], 
                            bayestestR::ci(out3$sims.list$slope_summer[,3], ci = 0.8)[[3]],
